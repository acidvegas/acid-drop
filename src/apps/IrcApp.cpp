...
void create(lv_obj_t* parent) {
    ...
    // Header row: home, reconnect, settings, info, then the window names
    ...
    // Builds one of the header buttons. They are all the same shape, and the
    // shape matters: these are finger targets on a 320px screen.
    auto makeButton = [&](const char* glyph, lv_color_t tint,
                          lv_event_cb_t onClick) -> lv_obj_t* {
        lv_obj_t* button = lv_obj_create(tabRow);
        lv_obj_remove_style_all(button);
        lv_obj_set_size(button, 38, 28);
        ...
        lv_obj_add_event_cb(button, onClick, LV_EVENT_CLICKED, nullptr);
        return button;
    };

    // Home first, and in the accent colour: it is the way out of this app and
    // the T-Deck keyboard has no escape key to fall back on.
    lv_obj_t* homeButton = makeButton(LV_SYMBOL_HOME, theme::accent(),
                                      [](lv_event_t*) { ui::home(); });
    lv_obj_set_style_bg_color(homeButton, lv_color_hex(theme::kAccentDim), 0);

    lv_obj_t* reconnect = makeButton(LV_SYMBOL_REFRESH, theme::textDim(),
                                     [](lv_event_t*) {
    ...
    makeButton(LV_SYMBOL_LIST, theme::textDim(),
               [](lv_event_t*) { openInfoPanel(); });

    // The window names take the remaining width, to the right of the buttons.
    s_tabs = lv_obj_create(tabRow);
    ...
    lv_obj_set_style_pad_hor(s_tabs, 2, 0);
